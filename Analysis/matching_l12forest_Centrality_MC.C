...
void matching_l12forest_Centrality_MC_new(){

  const TString l1_input = " /data/ginnocen/NtupleEmulatorMC/L1UpgradeAnalyzer_MC_9July2014.root";
  ...
  const TString forest_input = "/data/ginnocen/ForestMC/HiForest_HYDJET_Track8_Jet26_STARTHI53_LV1_merged_forest_0.root";
  ...
  TTree *fEvtGen = (TTree*)fFile->Get("HiGenParticleAna/hi");
  fTree->AddFriend(fEvtTree);
  fTree->AddFriend(fEvtGen);
  ...
  fTree->SetBranchAddress("ncoll",&ncoll);
  ...
  TFile*fouput=new TFile(Form("Results/CalibrationEtsum_CentLim_%d_%d_%d_%d_%d_8July2014_MC.root",limit0,limit1,limit2,limit3,limit4),"recreate"); 
  fouput->cd();
...
int main(){
  matching_l12forest_Centrality_MC_new();
  return 0; 
}
